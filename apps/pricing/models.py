...
class Transaction(models.Model):
    ...
    # RENAMED from flutterwave_ref -> provider_reference: this column now
    # stores whichever gateway's own reference (Paystack's `reference`,
    # Flutterwave's `tx_ref`), so it shouldn't be named after one gateway.
    # Needs `makemigrations` + `migrate` when applied to the real project
    # (Django will ask "did you rename a field?" -- answer yes so it's a
    # RenameField, not a drop+add that loses data).
